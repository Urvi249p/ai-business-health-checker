...
def get_tasks(agents: list, business_profile: dict, enriched_context: str = None) -> list[Task]:
    ...
    task2 = Task(
        ...
        """,
        expected_output=(
            "A detailed SWOT analysis with at least 4 specific "
            "evidence-based points per quadrant formatted as two "
            "separate Markdown tables: first table has columns "
            "Strengths and Weaknesses, second table has columns "
            "Opportunities and Threats. Use pipe | syntax only. "
    ...
    task3 = Task(
        ...
        """,
        expected_output=(
            "A pricing strategy recommendation with model choice, "
            "suggested price points, justification, and immediate "
            "action steps. Use Markdown headers, bullet points, "
            "and pipe tables only. No ASCII art, no code blocks, "
            "no backticks."
        ),
        context=[task1, task2],
    )
    ...
    task4 = Task(
        ...
        """,
        expected_output=(
            "A 90-day growth action plan structured as three "
            "phases using ### headers and bullet points. Each "
            "phase has specific actions, owners, metrics, and "
            "expected outcomes. No ASCII art, no code blocks, "
            "no tables for phase overview. Plain Markdown only."
        ),
    ...
    task5 = Task(
        agent=agents[4],
        description=f"""
            Assemble a complete professional business audit 
            report in Markdown format using the outputs from 
            all previous tasks.
            {{context_block}}
            ...
            ## Executive Summary
            (3-4 sentence overview of the business and 
            ...
            ## Key Recommendations
            (top 5 most important actions the business should 
            take, prioritized by impact)
            ...
            *Report generated by Business Audit AI*
            ...
            - Code blocks: do NOT use triple backticks ```
            - Inline code: do NOT use single backticks `
            - ASCII art or box diagrams made of ■ = - * # symbols
            - Flowcharts or diagrams made of text characters
            - Any box-drawing characters or decorative borders
            - HTML tags of any kind

            SWOT TABLE FORMAT — use exactly this structure:
            ...
            Never use "Strategic Dimension", "Positive/Helpful", 
            or "Negative/Harmful" as SWOT table headers.
            Always use exactly: Strengths, Weaknesses, 
            Opportunities, Threats as column headers.

            90-DAY PLAN FORMAT — use exactly this structure:
            ### Phase 1 — Days 1-30: [Phase Name]
            - **Action**: description
            ...
            Never wrap the phase overview in a table or 
            ASCII diagram. Use plain headers and bullets only.

            PRICING FORMAT:
            Use numbered sections with headers and bullet points.
            Price tables should use Markdown pipe tables only.
            Never use ASCII box diagrams for pricing tiers.

            Make it professional and ready to present to a 
            business owner. The report should feel like it was 
            written by a senior consultant, not generated by AI.
            Incorporate specific insights from the owner 
            interview. Make recommendations feel personalized 
            and grounded in real owner feedback.
        """,
        expected_output=(
